	...
	$(function() {
		...
		$('body').css({ background:'#222' }).prepend(
			div(style({ position:'absolute', top:0, left:0, width:'100%' }),
				img({ src:'/resources/status-bar.png' }, style({ width:320, display:'block', margin:'0 auto', position:'relative', top:34 })),
				img({ src:'/resources/iphone4.png' }, style({ margin:'-120px auto', display:'block' }))
			)
		)
	})
