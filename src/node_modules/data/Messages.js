...
function getPayload() {
	...
	var reverse = {}
	each(fields, function(encoding, name) {
		if (reverse[encoding]) { throw new Error("Duplicate encoding!") }
		reverse[encoding] = name
	})
	...
	return { fields:fields, encode:makeEncoder(fields), decode:makeEncoder(reverse), verify:verifyPayload }
}

function makeEncoder(fields) {
	return function encode(type, data) {
		var res = {}
		for (var key in data) {
